...
### 可选：给所有访问者配一个共享的 GitHub PAT

Vercel 是多人访问的无状态公网环境，匿名 API 限流（60/hr）非常容易被瞬间打满。可以在 **Project → Settings → Environment Variables** 加一个：

| Name | Value |
| --- | --- |
| `GITHUB_TOKEN` | `github_pat_xxx`（Fine-grained，Public Repositories read-only 即可） |
| `GITEE_TOKEN`（可选） | 你的 Gitee 令牌 |

- 这个 token **只在服务端使用**，不会下发到浏览器、不进 localStorage
- 用户仍可在 `/settings` 填自己的 token，**会优先覆盖**服务端内置 token
- 设置页会显示当前命中哪一路：🟢 你填写的 / 🔵 服务端内置 / 🟡 未配置
